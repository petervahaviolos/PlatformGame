...
public class GameManager extends GameCore {
	...
	private int score = 0;
	private int level = 1;
	private int time = gameLength;

	private GameAction moveLeft;
	private GameAction moveRight;
	private GameAction jump;
	private GameAction exit;
	private GameAction suicide;
	...
	private void initInput() {
		...
		suicide = new GameAction("suicide", GameAction.DETECT_INITAL_PRESS_ONLY);
		...
		inputManager.mapToKey(suicide, KeyEvent.VK_S);
	}
	...
	public void acquirePowerUp(PowerUp powerUp) {
		...
		if (powerUp instanceof PowerUp.Star) {
			...
			level++;
			map = resourceManager.loadNextMap();
		} else if (powerUp instanceof PowerUp.StartButton) {
			map = resourceManager.loadNextMap();
		}
	}
	...
	public void draw(Graphics2D g) {
		...
		renderer.draw(g, map, screen.getWidth(), screen.getHeight(), lives, score, level, time);
	}
}
